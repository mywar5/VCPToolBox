...
### 动态逻辑强化框架：从响应到预判的进化
- **智能意图分析**: VCP引入了一套动态逻辑强化框架，该框架基于Vcptavern插件生态，显著提升了AI在逻辑链反思、信息耦合、意图预判及动态工作流构建方面的能力。AI不再被动响应，而是能够主动分析用户潜在意图，并灵活构建VCP插件调用链以获取决策所需的完备信息。
...
- **持久化缓存与自动重算**：实现了 SVD 基底的持久化存储（SQLite kv_store），并建立了基于 Tag 库变化率（10% 阈值）的自动重算机制，兼顾性能与准确性。
**USearch 高性能向量索引**：
...
**`rust-vexus-lite`**：核心向量搜索引擎与语义计算核心
- **高性能原生 SVD**：引入基于 `nalgebra` 的 Rust 原生 SVD 实现，为 EPA 模块提供底层的正交基底构建支持，计算量从 Node.js 下沉至 Rust，极大提升初始化速度。
- **基于 USearch 的高性能 ANN 搜索**：采用业界领先的向量搜索技术，支持千万级数据。
- **扁平化 Buffer 传输**：采用高效的内存传输技术，减少 JS 与 Rust 之间的序列化开销。
...
- **`《《角色日记本》》` (插件) - 混合模式**:
  ...
  - **适用场景**: 大型日记库最推荐的日常使用模式。它既能避免无关信息的干扰，又能在相关时只提取最关键的记忆片段，最大限度地保证了上下文的精确性和高效性。
	...
	### 动态K值与语义动力学引力 (Semantic Dynamics)

	传统 RAG 往往只有固定的 K 值（检索数量）和死板的文本匹配。VCP RAG 引入了**语义动力学**，让检索过程像生物大脑一样具有“引力”和“惯性”。

	**TagMemo V3 核心进化：**
	- **上下文语义引力 (Contextual Gravity)**：通过**衰减聚合 (Decay Aggregation)** 算法，AI 会自动融合历史对话意图。越近的对话权重越高（默认衰减率 0.85），这形成了一种“语义引力”，让 AI 能够自动感知对话的主题变动，并保持极强的抗漂移能力。
	- **动态 K 值引擎 (K=3~10)**：K 值不再是死板的数字，而是由**逻辑深度 (L)** 和**共振强度 (R)** 共同驱动。
		- 当你与 AI 深入探讨某个专业话题（高逻辑深度）并产生跨领域关联（高共振）时，系统会自动将 K 值提升至 10，为你翻开更深层的背景知识。
		- 当对话较为琐碎或充满噪声时，K 值会自动收缩，避免无关信息干扰。
	- **意图融合 8:2 法则**：当前的查询向量由“当下意图”与“历史聚合意图”按 8:2 比例完美融合，确保 AI 既能快速响应你的新话题，又不会忘记刚才聊到哪了。
...
### Tag 向量网络牵引检索 (Tag Vector Network Retrieval) - VCP 原创算法 `浪潮RAG` V3

这是完全原创的、开创性的检索增强技术，在传统 RAG 基础上引入了全新的 "Tag 向量牵引层"，通过**三阶段空间变换**，从根本上提升知识库召回的精准度。V3 版本引入了**“自我认知”**能力，让 AI 具备了**“知道自己懂什么，也知道自己不懂什么”**的元认知能力。

**启用语法**：`[[角色日记本::TagMemo]]` (自动应用动态 β 权重) 或 `[[角色日记本::TagMemo0.65]]` (强制指定权重)

**V3 架构深度集成：**

1. **EPA (嵌入投影分析) - 思想的 X 光机**：
    - **私有语义坐标系**：通过对 Tag 库进行 K-Means 聚类和 SVD 分解，在 Rust 层构建了一套正交基底。
    - **逻辑深度 (L) 与信息熵**：量化查询的“聚焦程度”。AI 现在能感知到你的问题是深思熟虑的探讨，还是漫无目的的闲聊。
    - **跨域共振 (R)**：检测不同语义维度（如“技术”与“哲学”）之间的深层关联，这是触发高 K 值检索的关键。

2. **残差金字塔 (Residual Pyramid) - 知识盲区识别**：
    - **握手差值 (Handshake Delta)**：计算查询向量与已知 Tag 空间的差值，识别“已知”与“未知”。
    - **新颖度与噪声识别**：系统能区分什么是“需要学习的新知识”，什么是“无意义的噪声”。
    - **自我察觉**：当检测到高熵噪声时，AI 会自动降低 Tag 权重，甚至选择“闭嘴”，有效抑制幻觉。

3. **动态参数引擎 (β 融合)**：
    - 实现了基于 Sigmoid 函数的动态权重公式：`β = σ(L · log(1 + R) - S · noise_penalty)`。
    - 权重范围约 **0.05 ~ 0.45**（基准 0.15），根据对话质量实时波动，让检索结果始终处于“最舒适”的语义区间。
...
- **词元组捕网系统自学习**: 监控用户查询，自动发现并建议新的语义关联，不断扩张“词元捕网”的边界与精度。
	...
	通过 `[[VCP元思考:creative_writing::Group]]` 这样的高级语法，用户可以精细地控制思维链的思维主题和每一个簇环节，从使用的思考路径，到每个阶段的思考深度。这套系统的诞生，标志着VCP已从一个强大的“工具箱”，正式进化为一个能够进行结构化、可控化、且能自我迭代的“人工思维框架”。
	功能解析：
	1. creative_writing，当前思维包链条调用主题格式，由用户为其定义一个簇的递归顺序，例如“文风分析簇”，“剧情总结簇”，“提笔辩证簇”，“着笔润色簇”。不写入则为default，使用逻辑通用元配簇链条。写入Auto则为动态匹配主题。每个主题会使用完全不同的逻辑chunk簇。
	2. Group/Rerank，功能同日记本，是否开启词元组网或者重排扩寻。
	3. 在对应theme配置文件里定义2-1-1-1，定义每个簇的动态K上限，根据不同模式数组自定义。
	4. 逻辑单元簇：每个簇里都包含了大量的step chunk，它们都是特定语境下会使用到的逻辑推理单元，例如中文谐音梗冷笑话簇，内含18个step，会根据当前上下文的冷笑话类型语义级推送2-4个step到逻辑簇，方便AI构建正确的理解思维模式。
	5. 通过[[VCP元思考::Auto::Group]]来自动路由多个thinktheme。为每一个theme设置单独的词元捕网以提高上下文匹配准确度。
...
### AI主观能动性：从被动响应到自主行动的飞跃

VCP-Agent拥有完整的自主主观能动性。VCP如何实现了这一点？

1.  **时间线规划行动 (AgentAssistant总线)**: Agent可以调用AgentAssistant总线，给“未来的自己打电话”，从而引发一个post递归，实现时间线规划行动。
2.  **自我心跳驱动 (FlowInvite总线)**: Agent可以调用一个FlowInvite总线，使用一个固定的“AI自己设计的提示词”，以一个“AI自己设定的时间间隔”不断post自己，从而实现自我行动。AI可以随时调整这个心跳总线。
3.  **跨Agent唤醒**: Agent可以被AgentAssistant里的别的Agent的通话唤醒。
    > (AgentAssistant和FlowInvite都是分布式混合插件，即便服务器总台离线，该插件仍可工作，AI甚至可以直接启动VCP服务器总台)
4.  **流式输出编辑**: Agent可以在流式输出里回溯/编辑已经发出的内容或者编辑已有的上下文内容。
...
- **设计哲学**: 允许在单条消息中通过特定语法将此消息切割为多条消息（上下文会话楼层），并且可以为每一条消息分配角色（role：system、user、assistant）。也可以不切割消息，仅仅实现将当前的消息角色直接改变。例如，当 AI 输出 <<<[ROLE_DIVIDE_SYSTEM]>>>…content…<<<[END_ROLE_DIVIDE_SYSTEM]>>> 后，未来此条消息在上下文中的角色就不再是 assistant 而是 system 了。
- **核心能力**:
    - **语法支持**:
        - `<<<[ROLE_DIVIDE_SYSTEM]>>> ... <<<[END_ROLE_DIVIDE_SYSTEM]>>>`: 将包裹的内容切割为 `SYSTEM` 角色消息。
        - `<<<[ROLE_DIVIDE_ASSISTANT]>>> ... <<<[END_ROLE_DIVIDE_ASSISTANT]>>>`: 将包裹的内容切割为 `ASSISTANT` 角色消息。
        - `<<<[ROLE_DIVIDE_USER]>>> ... <<<[END_ROLE_DIVIDE_USER]>>>`: 将包裹的内容切割为 `USER` 角色消息。
    - **工作机制**:
        - **极早触发**：角色分割在 VCP 接收到请求后的第一时间执行，甚至早于变量替换和多模态处理。
        - **全功能支持**：被切割出来的楼层（如 `SYSTEM` 角色）将完整支持 VCP 后续的解析功能（如变量替换、RAG 注入等）。
        - **切割逻辑**：将被语法包裹的内容之前的内容切割为一个本楼层原先所属 role 的楼层 A；将被语法包裹的内容切割为一个指定 role 的楼层 B；将被语法包裹的内容之后的内容切割为一个本楼层原先所属 role 的楼层 C。
        - 后续楼层 C 中如果还有相关切割语法则继续同理执行。
        - 处理完之后，分割标签本身将不出现在上下文中。
        - 嵌套时，仅仅解析最外层的 `<<<[ROLE_DIVIDE_xxx]>>> ... <<<[END_ROLE_DIVIDE_xxx]>>>`。
    - **配置**:
        - `EnableRoleDivider`: 在 `config.env` 中设置为 `true` 以启用此功能（总开关）。
        - `RoleDividerSystem`: 是否允许切割为 `SYSTEM` 角色（默认 `true`）。
        - `RoleDividerAssistant`: 是否允许切割为 `ASSISTANT` 角色（默认 `true`）。
        - `RoleDividerUser`: 是否允许切割为 `USER` 角色（默认 `true`）。
        - `RoleDividerIgnoreList`: 在 `config.env` 中配置忽略列表（JSON 数组）。当包裹的内容（忽略换行、空格、反斜杠后）在此列表中时，忽略分割处理并保留标签。
    - **保护机制**:
        - **SystemPrompt 保护**: 对话历史中的第一条消息（通常是系统提示词）永远不会被分割。
        - **语法块保护**: 被 `<<<[TOOL_REQUEST]>>>` 与 `<<<[END_TOOL_REQUEST]>>>` 或 `<<<DailyNoteStart>>>` 与 `<<<DailyNoteEnd>>>` 包裹的内容不会被解析为角色分割符。
